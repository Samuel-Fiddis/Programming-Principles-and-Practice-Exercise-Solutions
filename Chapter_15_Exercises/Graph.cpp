...
Flex_fct::Flex_fct(Fct f, double r1, double r2, Point xy,
	int count, double xscale, double yscale)
	: r1(r1),r2(r2),orig(xy),count(count),xscale(xscale),yscale(yscale)
	// graph f(x) for x in [r1:r2) using count line segments with (0,0) displayed at xy
	// x coordinates are scaled by xscale and y coordinates scaled by yscale
{
	if (r2 - r1 <= 0) error("bad graphing range");
...
//void Flex_fct::set_range(double range1, double range2)
//{
//	r1 = range1;
//	r2 = range2;
//	if (r2 - r1 <= 0) error("bad graphing range");
...
void Flex_fct::set_origin(Point p)
{
	if (r2 - r1 <= 0) error("bad graphing range");
	if (count <= 0) error("non-positive graphing count");
	double dist = (r2 - r1) / count;
	double r = r1;
	for (int i = 0; i<count; ++i) {
		set_point(i, Point(point(i).x + p.x - orig.x,point(i).y + p.y - orig.y));
		r += dist;
	}
	orig = p;
}

void Flex_fct::set_scale(double xscale1, double yscale2)
{
	if (r2 - r1 <= 0) error("bad graphing range");
	if (count <= 0) error("non-positive graphing count");
	double dist = (r2 - r1) / count;
	double r = r1;
	for (int i = 0; i<count; ++i) {
		set_point(i, Point(((point(0).x - orig.x) / xscale)*xscale1 + orig.x, ((point(0).y - orig.y) / yscale)*yscale2 + orig.y));
		r += dist;
	}
	xscale = xscale1;
	yscale = yscale2;
...
void Flex_fct::set_fct(Fct fun)
{
	...
	double r = r1;
	for (int i = 0; i<count; ++i) {
		set_point(i,(Point(orig.x + int(r*xscale), orig.y - int(fun(r)*yscale))));
		r += dist;
	}
}
...
Bar_graph::Bar_graph(vector<double> N, Point xy, double xlength, double yscale)
	: N(N), orig(xy), xlength(xlength), yscale(yscale)
{
	for (int i = 0; i < N.size(); ++i)
	{
		bars.push_back(new Rectangle{ Point{orig.x + int((1 + 2 * i)*xlength / (2 * N.size())) - 1,orig.y - int(N[i] * yscale)},int(xlength / (2 * N.size())) - 1,int(N[i] * yscale) });
	}
}
...
void Bar_graph::set_bar_color(int n, Color c) {
	if(n<bars.size())
	bars[n].set_fill_color(c);
}

void Bar_graph::label_bar(int n, string t) {
	labels.push_back(new Text{ Point{ int(bars[n].point(0).x),int(bars[n].point(0).y + bars[n].height() + 12) }, t });
}
